...
class Benchmark():
    ...
    def get_hypervolume(self, pareto_points, thresholds):
        ...
        (if None it takes automatically the minimum of each objective in
        the ground truth).
        ...
        # NOTE: THIS IS SCALED ALWAYS W.R.T. THE GROUND TRUTH.
    ...
    def get_objective_values(self, df):  # Maximizing.
        """ Get a list of objective values (for instance yields and cost)."""
        predata = []
    ...
    def get_predictions_errors(self):
        ...
        if os.path.exists(f'pred_{self.filename}'):
            ...
            for objective in self.objective_names:
                ...
                y_var = df_preds[f"{objective}_predicted_variance"].values
    ...
    def run(self, steps, batch, seed=0,
        ...
        for step in range(0, steps):
            ...
            df_run = self.edbo.run(
                    ...
                    acquistion_function=self.acquisition_function,
    ...
    def _plot_predictions(self):
        ...
        for i in self.objective_names:
            ...
            g1 = sns.heatmap(ground, cmap='Spectral_r', square=True,
            ...
            g2 = sns.heatmap(pred_mean, cmap='Spectral_r', square=True,
                             xticklabels=[], yticklabels=[],
                             linewidths=0.7, cbar=False,
                             cbar_kws=dict(use_gridspec=False, location="bottom",
                                           ),
    ...
    def _plot_ground_2d(self):
        ...
        ax.add_patch(
            patches.Rectangle(
                tuple(self.min_bounds),  # (x,y)
                self.max_bounds[0],  # width
                self.max_bounds[1],  # height
                facecolor="red",
                alpha=0.1
            )
        )
