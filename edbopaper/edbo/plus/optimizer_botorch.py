...
class EDBOplus:
    ...
    def _model_run(self, data, df_train_x,  df_test_x, df_train_y, batch,
        ...
        for i in range(0, n_objectives):
            train_x = torch.tensor(init_train).double().to(**tkwargs)
            train_y = np.array(y)[:, i]
            train_y = (np.atleast_2d(train_y).reshape(len(train_y), -1))
            train_y_i = torch.tensor(train_y.tolist()).double().to(**tkwargs)
        ...
        print('Number of QMC samples:', sobol_num_samples)
        ...
        if acquisition_function == 'noisyEHVI':
            sampler = SobolQMCNormalSampler(num_samples=sobol_num_samples, collapse_batch_dims=True)
            partitioning = NondominatedPartitioning(
                ref_point=ref_point,
                Y=torch.tensor(y)).double().to(**tkwargs)
        ...
        # best_samples_scores = acq_result[1].detach().cpu().numpy()
