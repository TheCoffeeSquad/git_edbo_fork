...
class EDBOplus:
    ...
    def _init_sampling(df, batch, sampling_method, seed):
        ...
        for sample in samples.to_numpy():
            ss = np.argwhere((df_sampling_matrix == sample).all(axis=1))[0][0]
            selected_samples.append(ss)

        priority = np.zeros_like(df_sampling.index)
        priority[selected_samples] = 1
    ...
    def run(self,
            ...
            get_predictions=True, acquistion_function='EHVI',
            sigma_uncertainty=1.,
            continuous_features=True,
            add_random_samples=True):

        """
        objectives (list of str): list containing the name for each objective.
        Example:
            objectives = ['yield', 'cost', 'impurity']

        objective_mode (list of str): list to select whether the objective
        should be maximized or minimized. Examples:
        A) Example for single-objective optimization:
            objective_mode = ['max']
        B) Example for multi-objective optimization:
            objective_mode = ['max', 'min', 'min']

        objective_threshold (list of floats): list of worst case values for
        each objective. Example:
            objective_threshold = [50.0, 10.0, 10.0]

        columns_features (list of str): list containing the names of the
        columns to be included in the regression model. By default set to
        'all', which means the algorithm will automatically select all the
        columns that are not in the *objectives* list.

        batch (int): number of experiments that you want to run in parallel.
            For instance *batch = 5* means that you will run 5 experiments in
            each EDBO run. You can change this number at any stage of the
            optimization, so don't worry if you change your mind after creating
            or initializing the reaction scope.

        get_predictions (bool): if True it will print out a *csv file* with
        the predictions. You can also access the *predicted_mean* and
        *predicted_variance* through the EDBOPlus class.

        directory (str): directory to save your scope and priority list

        filename (str): filename to save a *csv* with the priority list. If
        *get_predictions=True* it will automatically save a second file
        including the predictions (*pred_filename.csv*).

        init_sampling_method (str): Method for selecting the first samples in
        the scope (in absence)  Choices are:
            - 'seed' : Random seed (as implented in Pandas).
            - 'lhs' : LatinHypercube sampling.
            - 'cvtsampling' : CVT sampling.

        seed (int): seed for the random initialization.
    ...
    def _model_run(self, data, df_train_x,  df_test_x, df_train_y, batch,
                   objective_mode, objective_thresholds, ohe_features,
        ...
        if n_objectives > 1:
            ...
            for q in range(0, batch):
                ...
                #####################################
                #####################################
                # Remove from test set the samples that have been already suggested.
                # tb = torch.tensor(acq)
                # mask_test = (~((tb == test_x).all(dim=1)))
                # n_features = len(test_x[0])
                # mask_test = mask_test.reshape(len(mask_test), -1).repeat(1, n_features)
                # test_x = torch.masked_select(test_x, mask=mask_test).reshape(-1, n_features)
                #####################################
                #####################################
                ...
                y_pred = bigmodel.posterior(
                    torch.tensor([acq])).mean.detach().numpy()[0].tolist()
                ...
                if get_predictions is True and q == 0:
                    ...
                    # pareto_test = pareto.eps_sort(
                    #         tables=self.predicted_mean[0],
                    #         objectives=np.arange(n_objectives),
                    #         maximize_all=True)
                    # pareto_positions = []
                    # for p in pareto_test:
                    #     b = np.argmin(cdist([p], self.predicted_mean[0]))
                    #     pareto_positions.append(data_scaled[b])
                    # pareto_test = np.array(pareto_test)
                    #
                    # if len(pareto_test) > 2:
                    #     tradeoff = HighTradeoffPoints()
                    #     tradeoff_args = tradeoff.do(-pareto_test)  # Always minimizing.
                    #
                    #     if tradeoff_args is not None:
                    #         for t in tradeoff_args:
                    #             print('ADDING TRADEOFF!!')
                    #             best_samples.append(pareto_positions[t])
                    ...
                    for i in range(0, n_objectives):
                        ...
                        self.ei[0][:, i] = ei_i
                        # print("-----------------")
                        # print(f"Predicted data for {self.objective_names[i]}:")
                        # print(f"Best collected sample: {y_best}")
                        # print(f"Best predicted mean: {pred_best}")
                        # print(f"Max. EI: {np.max(ei_i)}")
                        # print(f"Predicted mean for max. EI: {self.predicted_mean[0][:,i][np.argmax(ei_i)]}")
                        # print(f"Uncertainty for max. EI: {self.predicted_variance[0][:,i][np.argmax(ei_i)]}")
        ...
        if n_objectives == 1:  # Single-objective
            ...
            for q in range(0, batch):
                ...
                model, likelihood = build_and_optimize_model(train_x=train_x,
                                                 train_y=train_y,
                                                 ohe_features=ohe_features)
                ...
                if get_predictions is True and q == 0:
                    ...
                    self.ei[0, :] = e_i_res
                    # print("-" * 10)
                    # print(f"Predicted data for {self.objective_names}:")
                    # print(f"Best collected sample: {y_best}")
                    # print(f"Best predicted mean: {pred_best}")
                    # print(f"Max. EI: {np.max(ei_i)}")
                    # print(
                    #     f"Predicted mean for max. EI: {self.predicted_mean[0][np.argmax(ei_i)]}")
                    # print(
                    #     f"Uncertainty for max. EI: {self.predicted_variance[0][np.argmax(ei_i)]}")
        ...
        #
        # # # Add max uncertainty samples.
        # if add_random_samples:
        #     max_uncertainty_samples = np.max(self.predicted_variance[0], axis=1)
        #     argsort_uncertainty_descending = np.argsort(max_uncertainty_samples)[::-1]
        #     idx_unc = 0
        #     while (np.array(priority_list) >= 0.5).sum() < batch:
        #         print('Adding extra max. uncertainty samples.')
        #         random_c = argsort_uncertainty_descending[idx_unc]
        #         if priority_list[random_c] == 0:
        #             priority_list[random_c] = 0.5
        #         idx_unc += 1
        #         print('selected', (np.array(priority_list) >= 0.5).sum())
